...
void BM_add(benchmark::State& state) {
    srand(1); // 設定隨機數種子
    const unsigned int N = state.range(0); // 取得測試向量大小
    std::vector<unsigned long> v1(N), v2(N); // 建立兩個測試向量
    // 初始化向量元素為隨機值
    for (size_t i = 0; i < N; ++i) {
        v1[i] = rand();
        v2[i] = rand();
    }
    unsigned long* p1 = v1.data(); // 取得向量1的原始指標
    unsigned long* p2 = v2.data(); // 取得向量2的原始指標
    // 執行基準測試迴圈
    for (auto _ : state) {
        unsigned long a1 = 0, a2 = 0; // 累加器
        // 執行向量加法並累加結果
        for (size_t i = 0; i < N; ++i) {
            a1 += p1[i] + p2[i];
        }
        ...
        benchmark::ClobberMemory(); // 強制記憶體屏障
    }
    state.SetItemsProcessed(N*state.iterations()); // 設定處理項目數
