...
/* Only compile network functions if networking commands are enabled */
#if CFG_FTPGET || CFG_FTPPUT || CFG_HOST || CFG_HTTPD || CFG_IFCONFIG || \
    CFG_MICROCOM || CFG_NETCAT || CFG_NETSTAT || CFG_PING || CFG_RFKILL || \
    CFG_SNTP || CFG_TELNET || CFG_TELNETD || CFG_TFTP || CFG_TFTPD || \
    CFG_TUNCTL || CFG_WGET
