...
void print_stats(double elapsed_seconds, int num_orders) {
    ...
    printf("\n=== HFT-Optimized C Client Performance (Simple) ===\n");
    printf("Optimizations: NUMA, Memory Locking, CPU Affinity\n");
...
int main(int argc, char* argv[]) {
    ...
    printf("=== HFT-Optimized C Client (Simple) ===\n");
    printf("Using %d concurrent connections\n", num_connections);

    // HFT Optimization 1: Initialize NUMA
    if (numa_available() >= 0) {
        numa_set_localalloc();
        numa_set_strict(1);
        printf("✓ NUMA initialized: local allocation enabled\n");
    }

    // HFT Optimization 2: Lock memory for HFT
    if (mlockall(MCL_CURRENT | MCL_FUTURE) == 0) {
        printf("✓ Memory locked: mlockall(MCL_CURRENT | MCL_FUTURE)\n");
    } else {
        fprintf(stderr, "Warning: mlockall failed: %s\n", strerror(errno));
        fprintf(stderr, "Hint: Run with sudo or set CAP_IPC_LOCK capability\n");
    }
