...
**對比 C pthread（OS Thread）：**

| 特性 | Goroutine | C pthread（Linux 常見） |
|------|-----------|-----------|
| 堆疊大小 | 初始約 `2KB`，可動態增減 | 預設通常較大（Linux/glibc 常見 `8MB`），可調整 |
| 建立與銷毀 | 成本較低（主要在 user-space） | 成本較高（需核心參與） |
| 切換成本 | 通常較低 | 通常較高 |
| 典型併發規模 | 高（常見可到十萬級，受資源限制） | 較低（受每線程資源與排程成本限制） |
| 調度器 | Go Runtime（G-M-P / M:N） | OS Kernel（1:1） |

**補充：**
- Goroutine 的建立通常不需要每次進入核心態，但 runtime 需要擴增 M 時仍可能觸發系統呼叫。
- 切換時間會受硬體、負載、搶占與快取命中率影響，不宜用固定常數描述。
...
4. **Goroutine 執行過久：**
   - Go 1.14+ 使用基於信號的搶佔式調度
   - 每個 goroutine 執行約 10ms 後可能被搶佔
