...
- [第六部分：Go Runtime 深入與面試高頻題](#第六部分go-runtime-深入與面試高頻題)
  - [1：Structural Typing 原理](#1-為什麼-dog-可以-assign-給-speakerstructural-typing)
  - [2：Channel 內部結構（hchan）](#2-channel-內部結構hchan)
  - [3：GMP 排程器深入](#3-gmp-排程器深入)
  - [4：Map 內部結構](#4-map-內部結構)
  - [5：Context 深入](#5-context-深入取消傳播與最佳實踐)
  - [6：Closure 與 Goroutine 陷阱](#6-closure-與-goroutine-的經典陷阱)
  - [7：sync.Mutex 狀態機](#7-syncmutex-內部狀態機)
  - [8：string 內部結構](#8-string-內部結構與-stringsbuilder)
...
> **面試回答：** channel 底層是 `hchan` 結構，包含一個環形 buffer、兩個等待佇列（sendq/recvq）和一把 mutex。發送/接收時，優先直接交給對方（不經 buffer），其次走 buffer，最後才 park goroutine。

---

## 3. GMP 排程器深入

GMP 是 Go runtime 排程器的核心模型，也是 Go 面試的**必考題**。
...
> **面試回答模板：**
> GMP 是 Go 的 M:N 排程模型。G 是 goroutine，M 是 OS thread，P 是邏輯處理器（數量 = GOMAXPROCS）。M 必須綁定 P 才能執行 G。排程器從 local queue → global queue → work stealing 的順序找 G。syscall 時 P 會被 hand-off 給其他 M。Go 1.14 引入基於 SIGURG 的非同步搶佔，解決 tight loop 餓死問題。
...
> **面試回答：** 閉包捕獲的是變數的**參考**（位址），不是值的**複製**。在 for loop 中啟動 goroutine，如果直接引用迴圈變數，所有 goroutine 會共用同一個變數。修正方式是用函數參數傳入（複製值）或用 `:=` shadow。Go 1.22 改變了 loop variable 的語意，每次迭代都是新變數。

---

## 7. sync.Mutex 內部狀態機

Mutex 看起來簡單，但內部有精心設計的**飢餓模式**。
...
6. **Runtime 深入與面試高頻題**：Channel 內部結構（hchan）、GMP 排程器（work stealing + 非同步搶佔）、Map 內部結構（bucket + 漸進式擴容）、Context 取消傳播、Closure 陷阱、sync.Mutex 飢餓模式、string 底層結構。每題附記憶體結構圖和面試回答模板。
