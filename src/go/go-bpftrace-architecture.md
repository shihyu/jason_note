...
`bpftrace` 是 Linux 上基於 **eBPF (extended Berkeley Packet Filter)** 的高階追蹤工具，讓你在不修改程式碼、不重新編譯、不重啟程式的情況下，動態觀察任何正在執行的程式。

```
傳統觀察方式（需要改動程式）：
  程式碼 ──> 加 printf/log ──> 重新編譯 ──> 重啟程式 ──> 觀察輸出
...
**uprobe (User-space Probe)** 是 Linux 核心提供的機制，用來在使用者空間程式的任意位址插入「偵測點」。
...
| 目標 | 說明 | 觀察對象 | 需要 sudo |
|------|------|----------|-----------|
| `make trace-alloc` | heap 分配大小分佈 + Allocate 呼叫時間 | `main.Allocate` + `runtime.mallocgc` | 是 |
...
| `uprobe:檔案:前綴.*` | 萬用字元，比對所有符合前綴的符號 | `uprobe:./myapp:main.*` |
| `probe` | 當前命中的 probe 完整名稱 | `printf("%s", probe)` |
| `elapsed` | 程式啟動後的奈秒數 | `elapsed / 1000000` 換算毫秒 |
