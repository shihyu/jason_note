...
當我們在一台機器上運行上面的程式碼時，你會注意到，只要函數 raise_exception() 被呼叫，目標函數 run() 就會結束。這是因為一旦引發異常，程序控制就會跳出 try 塊，run() 函數將終止。之後可以呼叫 join() 函數來終止執行緒。在沒有函數 run_exception() 的情況下，目標函數 run() 將一直運行，並且永遠不會呼叫 join() 函數來終止執行緒。  
