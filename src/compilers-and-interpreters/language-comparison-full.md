...
## 目錄
1. [編譯模型與運行時系統](#一編譯模型與運行時系統)
2. [併發機制的編譯器實作](#二併發機制的編譯器實作)
3. [記憶體管理的編譯時與運行時策略](#三記憶體管理的編譯時與運行時策略)
4. [多型機制的代碼生成策略](#四多型機制的代碼生成策略)
5. [錯誤處理的控制流實作](#五錯誤處理的控制流實作)
6. [反射與元編程的編譯時/運行時權衡](#六反射與元編程的編譯時運行時權衡)
7. [泛型與代碼特化](#七泛型與代碼特化)
8. [編譯工具鏈架構](#八編譯工具鏈架構)
9. [總結：語言設計的編譯器視角](#九總結語言設計的編譯器視角)
...
**編譯器轉換：**
1. 將 `go` 關鍵字轉換為 `runtime.newproc()` 呼叫
2. 捕獲閉包變數,分配堆疊空間
3. 生成函數指標
...
2. **Promise 對象**：管理協程生命週期
3. **Resume/Suspend 點**：`co_await` 位置成為狀態切換點
