 ...
 * 純 C 實作：float 陣列加法
 */
void add_float_c(const float *a, const float *b, float *result, size_t n) {
 ...
 * NEON 實作：float 陣列加法
 *
 * 使用 vaddq_f32 一次處理 4 個 float
 */
void add_float_neon(const float *a, const float *b, float *result, size_t n) {
    size_t i = 0;
 ...
 * 測試指定大小的資料
 */
void test_add_with_size(size_t num_elements, int iterations) {
    char size_str[64];
    ...
    // 分配記憶體（16-byte 對齊以利 NEON 載入）
    // 注意：Android API < 28 不支援 aligned_alloc，使用 memalign
    ...
    for (int i = 0; i < iterations; i++) {
        add_float_c(a, b, result_c, num_elements);
    }
    timer_stop(&timer);
    ...
    timer_start(&timer);
    for (int i = 0; i < iterations; i++) {
        add_float_neon(a, b, result_neon, num_elements);
    }
    timer_stop(&timer);
    ...
    // 顯示效能比較
    double speedup = calculate_speedup(time_c, time_neon);
    printf("  純 C 實作:   %8.3f ms\n", time_c);
    printf("  NEON 實作:   %8.3f ms\n", time_neon);
    printf("  加速比:      ");

    if (speedup >= 2.0) {
        printf(COLOR_GREEN "%.2fx ⚡ 優秀！\n" COLOR_RESET, speedup);
    } else if (speedup >= 1.5) {
        printf(COLOR_YELLOW "%.2fx 不錯\n" COLOR_RESET, speedup);
    } else {
        printf(COLOR_RED "%.2fx 需改進\n" COLOR_RESET, speedup);
    }
 ...
 */
int main(void) {
    print_test_header("NEON 向量加法測試");

    printf("\n  測試說明:\n");
    printf("  - 比較純 C 與 NEON intrinsics 的效能\n");
    printf("  - NEON 使用 vaddq_f32 一次處理 4 個 float\n");
    printf("  - 測試不同資料量的效能表現\n");
    ...
    test_add_with_size(SIZE_1KB / sizeof(float), 10000);

    print_subtest_header("中資料量測試 (1MB)");
    test_add_with_size(SIZE_1MB / sizeof(float), 100);

    print_subtest_header("大資料量測試 (10MB)");
    test_add_with_size(SIZE_10MB / sizeof(float), 10);
