...
當然，根據編譯器的廢柴程度（你們大概應該知道我在說誰..）和項目的規模，編譯一次的時間代價可能會很大。這種情況下，我會拆分模塊，盡量只去編譯改過的模塊。如果這比較困難，那麼也可以使用`git worktree`來創建多個工作目錄：這樣你可以讓多個任務並行進行，互不幹擾，也算是彌補等待編譯所帶來的時間損失。
