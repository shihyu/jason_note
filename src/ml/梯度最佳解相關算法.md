...
Adagrad則是針對每個參數客制化的值，所以Ada這個字跟是Adaptive的簡寫，這邊假設 g_t,i為第*t*次第*i*個參數的梯度，
...
ε是平滑項，主要避免分母為0的問題，一般設定為1e-7。Gt這邊定義是一個對角矩陣，對角線每一個元素是相對應每一個參數梯度的平方和。這邊很有趣，一般文章都寫說分母是the sum of the squares of the gradients, i.e. θ*i* up to time step *t*，然後就沒有其他說明了。
