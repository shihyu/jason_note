...
核心挑戰在於馴服作業系統和硬體平臺，消除其引入的非確定性和額外延遲：

1. **作業系統排程不確定性**：預設的完全公平排程器（CFS）策略可能導致低延遲關鍵處理程序被背景任務（如日誌、監控）或核心執行緒（如 ksoftirqd、kworker）搶佔，引入不可預測的微秒級甚至毫秒級停頓。
...
2. 兄弟執行緒對 L1/L2 快取的存取會汙染或驅逐交易執行緒的熱點資料，增加快取遺失（Cache Miss）率
...
1. **中斷處理程序搶佔使用者態交易執行緒**，強制進行上下文切換
2. **中斷處理程序存取記憶體會汙染該核心的 L1d 快取**，導致交易執行緒的熱點資料被驅逐
...
即使進行了 CPU 綁定，預設情況下，核心執行緒（如負責軟中斷處理的 ksoftirqd，處理工作佇列的 kworker）仍然可能被調度到綁定的核心上執行，搶佔使用者態交易執行緒。此外，定期的時鐘中斷（Tick）和核心的 RCU（Read-Copy-Update）回調機制也會在隔離核心上產生開銷，破壞其執行的連續性。
...
這三者結合，為關鍵的低延遲任務提供了一個近乎獨佔的、不受核心內部機制幹擾的「靜默」執行環境。
...
在多路伺服器（NUMA 架構）中，記憶體控制器分佈在不同的物理 CPU 插槽（Node）上。CPU 存取其本地節點（Node）的記憶體（**Local Access**）速度最快，而存取其他節點（**Remote Access**）的記憶體需要通過 CPU 間的互連（如 Intel UPI、AMD Infinity Fabric），延遲顯著增加（通常高出 50%-100% 甚至更多）。
...
1. **防止頁快取回寫幹擾**：鎖定的匿名頁不會被標記為髒頁，避免了核心的週期性回寫執行緒在背景將這些頁寫回儲存而產生的記憶體頻寬爭用
