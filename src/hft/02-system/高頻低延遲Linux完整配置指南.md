...
在傳統 Linux 網路編程中，`poll()` 和 `recvfrom()` 是**兩個不同的系統呼叫**，各司其職：
...
雖然方式 2（非阻塞 recvfrom busy polling）和 DPDK 都是 busy polling，但**效率差異高達 1000 倍！**
...
> **重點**：這就是為什麼**即使都是 busy polling，DPDK 還是快得多**！不是概念的差異，而是實作層級的根本差異。
