...
- **作業系統排程不確定性**：預設的完全公平排程(CFS)策略可能導致低延遲關鍵行程被背景任務（如日誌、監控）或核心執行緒（如 ksoftirqd, kworker）搶佔，引入不可預測的微秒級甚至毫秒級停頓。
...
1. 若綁定到同一實體核上的兄弟執行緒（如日誌執行緒、監控執行緒）執行了計算密集型操作，尤其是使用AVX等寬指令集時，會完全佔用共享的浮點單元，導致交易執行緒被阻塞
2. 兄弟執行緒對L1/L2快取的存取會汙染或驅逐交易執行緒的熱點資料，增加快取未命中(Cache Miss)率

> 💡 **白話解釋**：
> - **問題1**：就像兩個人共用一臺電腦，一個在跑3D渲染（佔用顯卡），另一個想玩遊戲就卡住了。
...
停用超執行緒會降低系統的整體吞吐量(Throughput)。因此，合理的策略是**隔離關鍵路徑**：將交易策略引擎、網路處理執行緒等低延遲敏感任務**獨佔綁定到實體核心**（使用邏輯核ID的偶數或奇數部分），而將日誌記錄、監控上報、資料持久化等非即時性任務部署在啟用超執行緒的核心上。
...
1. **中斷處理程式搶佔使用者態交易執行緒**，強制進行上下文切換
2. **中斷處理程式存取記憶體會汙染該核心的L1d快取**，導致交易執行緒的熱點資料被驅逐
...
即使進行了CPU綁定，預設情況下，核心執行緒（如負責軟中斷處理的ksoftirqd，處理工作佇列的kworker）仍然可能被排程到綁定的核心上執行，搶佔使用者態交易執行緒。
...
在多路伺服器(NUMA架構)中，記憶體控制器分佈在不同的實體CPU插槽(Node)上。CPU存取其本地節點的記憶體(**Local Access**)速度最快，而存取其他節點(**Remote Access**)的記憶體需要透過CPU間的互連，延遲顯著增加（通常高出50%-100%甚至更多）。

> 💡 **白話解釋**：想像一個大公司有臺北和高雄兩個辦公室，每個辦公室都有自己的檔案室。臺北員工要查臺北檔案室的資料很快（本地存取），但要查高雄檔案室就得打電話請那邊傳真過來（遠端存取），慢很多。
...
> 💡 **白話解釋**：就像圖書館為了節省空間，把很久沒人借的書移到地下倉庫。當你突然要用時，得等管理員去地下室搬上來，很耗時。記憶體鎖定就是告訴圖書館「這些書永遠放在閱覽室，不準移走」。

`mlock`系統呼叫將指定的虛擬記憶體範圍**鎖定在實體記憶體(RAM)中**，確保其不會被換出。此外，它還具有兩個關鍵優勢：

1. **防止頁快取回寫幹擾**：鎖定的匿名頁不會被標記為髒頁
...
pqos -m llc:1 -t 10           # 監控COS1的LLC佔用
```
...
透過將高頻並行存取的關鍵資料結構按快取行大小(64位元組)進行記憶體對齊，確保它們獨佔一個或多個完整的快取行，可以徹底消除偽共享帶來的效能損耗。
