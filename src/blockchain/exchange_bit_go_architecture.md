...
- 交易所像「銀行櫃台」：負責記帳、審核提款、擋風險。
- BitGo 像「金庫 + 保全系統」：負責保管私鑰、簽名交易、把交易送上鏈。
- 區塊鏈像「公開總帳」：所有轉帳最後都要寫進去，誰都改不了。

入金白話：

- 交易所先跟 BitGo 要一個收款地址給用戶。
- 用戶把幣打進去後，等鏈上確認到一定次數。
- BitGo 通知交易所，交易所才在內部帳本加餘額。
- 但不能只信通知，還要每天拿「內部帳本 vs 鏈上餘額」做對帳。

提幣白話：

- 用戶申請提幣後，交易所先做 KYC/AML、風控、餘額檢查。
- 通過才送到 BitGo 簽名與廣播。
- 所以「能不能提」是交易所決定，「怎麼安全地提」是託管方決定。

錢包分層白話：

- 熱錢包放少量錢，專門應付即時提款（快但風險高）。
- 冷錢包放大部分資產，安全最高（慢但穩）。
- 中間再放一層 warm wallet 做緩衝，避免熱錢包一下被提空。
