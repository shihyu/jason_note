...
## Serverless vs VPS 核心概念

### 最核心的差異

你可以把兩者想像成：

- **VPS（24 小時運行）**：像是一台開著不關的電腦。程式碼（Daemon）就像在背景跑的軟體，隨時都在吃記憶體、佔用 CPU，但也隨時準備好處理資料（例如 24 小時連著 Binance WSS）。

- **Cloudflare Workers（觸發式）**：像是一個**感應燈**。只有有人走過（HTTP 請求）或鬧鐘響了（Cron Trigger），燈才會亮起（程式啟動），執行完任務後就立刻熄滅（釋放記憶體與 CPU）。

### 為什麼 Cloudflare 不能像 VPS 那樣「自己跑」？

1. **沒有持續的記憶體居住**：在 VPS 上，你的變數可以一直存在記憶體裡；在 Cloudflare，請求一結束，記憶體就會被清空。

2. **計費邏輯不同**：VPS 是按「時間」收費（租一個月多少錢）；Cloudflare 是按「次數」收費（執行一百萬次多少錢）。如果它讓你 24 小時佔用資源卻不間斷，那它就虧大了。

3. **基礎架構限制**：Cloudflare 的設計是為了處理極高併發（例如一秒鐘進來一萬個請求），而不是為了處理單個長時間的任務。

### 除了「點連結」和「Cron」，還有什麼能觸發它？

雖然它需要觸發，但觸發源其實很多：

| 觸發方式 | 說明 |
| :--- | :--- |
| **HTTP 請求** | 你點連結，或是別的程式打 API 給它 |
| **Cron Triggers** | 設定排程（例如每分鐘執行） |
| **Queue（隊列）** | 當有訊息進到 Cloudflare Queue 時觸發 |
| **Email** | 當有信件寄到特定地址時觸發 |

### 需要 24/7 常駐的任務怎麼辦？

如果你需要 24 小時連著 Binance WSS 報價，Cloudflare 會因為「沒有外部持續觸發」且「不允許長駐進程」而斷線。

**最省錢的替代方案建議：**

| 方案 | 說明 | 限制 |
| :--- | :--- | :--- |
| **Oracle Cloud 免費 VPS** | 免費方案提供 4 核心、24GB 記憶體，是目前最強的免費 24 小時 VPS | 需要申請，有時審核嚴格 |
| **GitHub Actions** | 雖然是觸發式，但一個 Task 最長可以跑 6 小時（非官方用途，有風險） | 不是設計來跑常駐服務的 |
| **Zeabur / Railway** | 這類平台有專門給「長駐程式（Background Worker）」跑的方案，比 Cloudflare 更適合跑機器人 | 免費額度有限，超過需付費 |
