...
class ScalarActuatorCubit extends DeviceActuatorCubit {
  ...
  void scalar(double value) {
    ...
    print("[INTIFACE_LOG] Dart DeviceActuator - 發送振動指令到設備 ${_device.name}, 索引: $_index, 強度: ${value / stepCount.toDouble()}");
...
class RotateActuatorCubit extends DeviceActuatorCubit {
  ...
  void rotate(double value) {
    ...
    _currentValue = value;
    emit(DeviceActuatorStateUpdate(_currentValue));
    print("[INTIFACE_LOG] Dart DeviceActuator - 發送旋轉指令到設備 ${_device.name}, 索引: $_index, 速度: ${(value / stepCount.toDouble()).abs()}, 順時針: ${value >= 0}");
