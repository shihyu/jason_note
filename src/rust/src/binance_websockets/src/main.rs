#!DepthOrderBookEvent[allow(dead_code)]
...
fn multiple_streams() {
    let endpoints =
        // ["BTCUSDT", "BNBETH"].map(|symbol| format!("{}@depth@100ms", symbol.to_lowercase()));
        ["BTCUSDT"].map(|symbol| format!("{}@depth@100ms", symbol.to_lowercase()));

    let keep_running = AtomicBool::new(true);
    let mut web_socket = WebSockets::new(|event: WebsocketEvent| {
        if let WebsocketEvent::DepthOrderBook(depth_order_book) = event {
            println!("{:?}", depth_order_book);
            // print_type_of(&depth_order_book);
            println!("{:?}", depth_order_book.bids);
            println!("{:?}", depth_order_book.asks);
            println!("{:?}", depth_order_book.asks[0]);
            print_type_of(&depth_order_book.bids);
        }
