...
當我們開發 Rust 應用程序時，有時我們需要減少更改、編譯和執行週期的時間。這聽起來有點複雜，但今天我將向你展示一個工具，它以一種非常簡單和自動的方式來實時監聽代碼變化。這個工具被稱為 cargo watch，它減少了項目變更、編譯、運行週期的時間。
...
我們從創建一個新項目開始。對於我們的例子，我們將其稱為 cargo-watch-example：
...
前面的例子是觀察整個項目或特定目錄上的變化的最簡單的配置，但你可以做的事情更多，這裡是命令說明：
