...
fn insert_tests() {
    ...
    let no_of_inserted = insert_into(posts)
        .values((
            title.eq("Diesel 教學"),
            body.eq("For this guide, we’re going to walk through CRUD,..."),
        ))
        .execute(&mut conn);
    ...
    let new_posts = vec![
        NewPost {
            title: "Diesel 教學".to_string(),
            body: "AAA".to_string(),
        },
        NewPost {
            title: "Rust 教學".to_string(),
            body: "BBB".to_string(),
        },
    ];
    let inserted_records = insert_into(posts)
...
fn delete_tests() {
    ...
    delete(posts).filter(body.eq("AAA")).execute(&mut conn);
}
