...
上面例子中把宏定義挪到 `main` 函數之前或者 `main` 函數裡面 `bar!(a)` 調用上面，就可以正常編譯運行。
...
看著很複雜的宏，細細分析下來是不是很簡單，不要被這些符號幹擾了

### 遞歸(recursion)

除了重複之外，宏還支持遞歸，即在宏定義時調用其自身，類似於遞歸函數。因為rust的宏本身是一種模式匹配，而模式匹配裡麵包含遞歸則是函數式語言裡面最常見的寫法了，有函數式編程經驗的對這個應該很熟悉。下面看一個簡單的例子：
