...
struct thread* rr_sched_choose_thread(void)
{
        ...
        /* else dequeue the first thread in ready queue */
                ...
                struct list_head* choose_thread_node =
                        rr_ready_queue_meta[smp_get_cpu_id()].queue_head.next;
                thread = list_entry(
                        choose_thread_node, struct thread, ready_queue_node);
...
int rr_sched(void)
{
        ...
        /* Some condition like `current_thread == NULL`, we don't need to do
         * anything */
        if (current_thread == NULL || current_thread->thread_ctx == NULL) {
        }
        /* Else if current_thread is exiting, set its state, thread_exit_state
           and don't put it into ready queue */
        ...
        /* Else if current_thread isn't waiting, enqueue current_thread to ready
           queue */
