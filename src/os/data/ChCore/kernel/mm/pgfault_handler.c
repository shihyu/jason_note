...
static void alloc_page_in_memory(void* pgtbl, vaddr_t va, paddr_t pa,
                                 vmr_prop_t perm)
{
    u64 min_time = -1;
    u64 alloc_index;
    ...
    if (is_mapped[alloc_index]) {
        ...
        memcpy(phys_to_virt(origin_pa[alloc_index]),
               phys_to_virt(simulate_pa + alloc_index* PAGE_SIZE), PAGE_SIZE);
        map_range_in_pgtbl(pgtbl, origin_va[alloc_index], origin_pa[alloc_index],
                           PAGE_SIZE, origin_perm[alloc_index]);
    }
    ...
    memcpy(phys_to_virt(simulate_pa + alloc_index* PAGE_SIZE), phys_to_virt(pa),
           PAGE_SIZE);
    map_range_in_pgtbl(pgtbl, va, simulate_pa + alloc_index* PAGE_SIZE, PAGE_SIZE,
                       perm);
}
