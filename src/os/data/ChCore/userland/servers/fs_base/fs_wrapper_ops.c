                    ...
                    struct fs_request* fr)
{
    ...
    ret = server_ops.open(path, flags, mode, & vnode_id, & vnode_size, & vnode_type,
                          & vnode_private);
    ...
    if (NULL != vnode) {
        ...
        assign_entry(server_entrys[entry_id], flags, entry_off, (void*)strdup(path),
                     vnode);
    } else {
...
int fs_wrapper_write(struct ipc_msg* ipc_msg, struct fs_request* fr)
{
    ...
    ret = 0;
    fd = fr->write.fd;
    buf = (void*)fr + sizeof(struct
                             fs_request);  // fixed + 8 bug; original bug without +8
