...
int real_file_writer(char* buffer, unsigned long file_page_idx,
                     int page_block_idx, void* private)
{
    struct fs_vnode* vnode;
    unsigned long long offset;
    size_t size;
...
void fs_wrapper_set_server_entry(u64 client_badge, int fd, int fid)
{
    ...
    for_each_in_list(private_iter, struct server_entry_node, node,
...
void translate_fd_to_fid(u64 client_badge, struct fs_request* fr)
{
    ...
    case FS_REQ_GETDENTS64:
        fr->getdents64.fd = fs_wrapper_get_server_entry(client_badge,
                            fr->getdents64.fd);
        break;
