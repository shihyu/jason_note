...
**為什麼是真實順序：** bpftrace 使用 `uprobe` 機制，在 **kernel 層級**動態攔截函數呼叫。當程式執行到某個函數的入口地址時，CPU 觸發斷點，kernel 通知 bpftrace 記錄事件。因此：

- `elapsed / 1000` 是真實的微秒級時間戳
- 順序編號反映的是**該函數第一次被呼叫**的先後順序（`@seen` map 負責去重）
- 整個追蹤過程不需要修改原始程式碼，也不需要重新編譯
...
| **FILE:LINE 是定義位置** | 合併的檔名行號來自 `go tool objdump` 萃取的 DWARF 資訊，顯示的是函數**定義**在原始碼的哪個位置，而不是「從哪裡被呼叫的」呼叫點 |
| **多執行緒交錯** | Go runtime 會啟動多個 OS thread（M），例如 sysmon 監控執行緒。輸出後半段可能出現不同 thread 的函數交錯出現，這也是真實的並行執行順序 |

> **總結**：一個 Go Hello World 程式從 ELF 入口點 `_rt0_amd64_linux` 開始，經過 450 個 runtime 初始化函數，最後才到達 `main.main`。這份輸出完整呈現了從組合語言入口 → runtime 初始化 → 標準庫初始化 → 使用者程式碼的真實執行路徑。
