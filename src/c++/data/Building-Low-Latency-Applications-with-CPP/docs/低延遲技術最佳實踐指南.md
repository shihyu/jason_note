...
檢查包含 `.cpp` 與 `.h`，並針對以下關鍵低延遲技術點補齊註解：

- 原子操作與記憶體序
- 分支預測提示
- 高精度計時（RDTSC/奈秒）
- Epoll/Socket/非阻塞 I/O
- 物件池/Placement New/對齊
- 綁核與執行緒親和性
...
- **分配延遲**：< 20ns（vs malloc 的 50-10000ns）
- **零碎片化**：所有物件大小相同
- **可預測延遲**：無動態分配的變異性
...
**文件版本**：1.0
**最後更新**：2026-01-11
**維護者**：Building Low Latency Applications Team
