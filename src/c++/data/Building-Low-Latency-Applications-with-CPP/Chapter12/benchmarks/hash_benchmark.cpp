...
template<typename T>
size_t benchmarkHashMap(T* order_book,
                        const std::vector<Exchange::MEClientRequest>& client_requests)
{
...
int main(int, char**)
{
    ...
    auto matching_engine = new Exchange::MatchingEngine(&client_requests,
...

    {
        ...
        std::cout << "ARRAY HASHMAP " << cycles << " CLOCK CYCLES PER OPERATION." <<
                  std::endl;
    }

    {
        auto me_order_book = new Exchange::UnorderedMapMEOrderBook(0, &logger,
                matching_engine);
        const auto cycles = benchmarkHashMap(me_order_book, client_requests_vec);
