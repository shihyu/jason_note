...
auto MarketDataConsumer::checkSnapshotSync() -> void
{
    ...
    logger_.log("%:% %() % Recovered % snapshot and % incremental orders.\n",
                __FILE__, __LINE__, __FUNCTION__,
                Common::getCurrentTimeStr(&time_str_), snapshot_queued_msgs_.size() - 2,
                num_incrementals);
    ...
    if (socket->next_rcv_valid_index_ >= sizeof(Exchange::MDPMarketUpdate)) {
        ...
        for (; i + sizeof(Exchange::MDPMarketUpdate) <= socket->next_rcv_valid_index_;
            ...
            if (UNLIKELY(in_recovery_)) {
                ...
                queueMessage(is_snapshot,
                             request); // queue up the market data update message and check if snapshot recovery / synchronization can be completed successfully.
            } else if (
                !is_snapshot) { // not in recovery and received a packet in the correct order and without gaps, process it.
                logger_.log("%:% %() % %\n", __FILE__, __LINE__, __FUNCTION__,
                            Common::getCurrentTimeStr(&time_str_), request->toString());
