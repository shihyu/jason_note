...
線程的創建者(例子中是主線程)負責管理線程的生命週期，所以讓我們來了解一下線程的生命週期。
...
解決數據競爭也非常容易：使用鎖或原子保護`valSleeper`。為了解決`valSleeper`和`std::cout`的生命週期問題，必須匯入線程而不是分離它。
...
為了更完整的瞭解`std::thread`，接下來了解其成員函數。
