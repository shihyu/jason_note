                               ...
                               struct mount_point_info_node* mount_point_info)
{
    ...
    for_each_in_list(private_iter, struct client_fd_info_node, node,
    ...
    struct client_fd_info_node* n = (struct client_fd_info_node*)malloc(sizeof(
...
void fsm_server_dispatch(struct ipc_msg* ipc_msg, u64 client_badge)
{
    ...
    case FS_REQ_MOUNT:
        ret = fsm_mount_fs(fr->mount.fs_path,
                           fr->mount.mount_path); // path=(device_name), path2=(mount_point)
        break;
    ...
    case FS_REQ_OPEN:
        ...
        ipc_msg_forward = ipc_create_msg(mpinfo->_fs_ipc_struct,
    ...
    case FS_REQ_WRITE:
        ...
        ipc_msg_forward = ipc_create_msg(mpinfo->_fs_ipc_struct,
                                         sizeof(struct fs_request) + fr->write.count + 1, 0);
        chcore_assert(ipc_msg_forward);
        fr_forward = (struct fs_request*) ipc_get_msg_data(ipc_msg_forward);
