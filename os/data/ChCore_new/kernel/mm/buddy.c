                               ...
                               struct page* page)
{
        ...
        while (true) {
                ...
                list_add(&first_page->node,
                         &pool->free_lists[order - 1].free_list);
                // second_page->order = order - 1;
                // second_page->allocated = 0;
                set_free_page(second_page, order - 1);
                list_add(&second_page->node,
                         &pool->free_lists[order - 1].free_list);
        ...
        while (match_order <= BUDDY_MAX_ORDER) {
                if (pool->free_lists[match_order].nr_free > 0) {
                        struct page* target_page = split_page(
                                pool,
                                order,
                                pool->free_lists[match_order].free_list.next);
...
static struct page* merge_page(struct phys_mem_pool* pool, struct page* page)
{
        ...
        while (true) {
                ...
                if (buddy_page == NULL || buddy_page->allocated == 1
                    || buddy_page->order != it_page->order
                    || it_page->order >= BUDDY_MAX_ORDER - 1) {
                        BUDDY_LOG("Bad Buddy!");
                        return it_page;
                }
                ...
                // it_page->order++;
                BUDDY_LOG("Merge into a bigger page: order->%d",
                          it_page->order);
        }
...
void buddy_free_pages(struct phys_mem_pool* pool, struct page* page)
{
        ...
        BUDDY_LOG("Add page (order: %d, allocated: %d) to freelist",
                  page->order,
                  page->allocated);
...
void show_buddy(struct page* buddy_page)
{
        if (buddy_page != NULL)
                BUDDY_LOG(
                        "Buddy: order->%d, allocated->%d, next->%llx, prev->%llx",
                        buddy_page->order,
                        buddy_page->allocated,
                        buddy_page->node.next,
                        buddy_page->node.prev);
}
