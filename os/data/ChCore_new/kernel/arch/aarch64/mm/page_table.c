                        ...
                        pte_t** pte, bool alloc)
{
        ...
        if (IS_PTE_INVALID(entry->pte)) {
                if (alloc == false) {
                        // PGTBL_LOG("Level %d pte invalid and alloc == false",
                        // level);
                        return -ENOMAPPING;
                } else {
                        ...
                        PGTBL_LOG(
                                "Alloc a new ptp in level %d, new ptp's paddr:%llx",
                                level,
                                new_ptp_paddr);
                        /* LAB 2 TODO 3 END */
                }
                       ...
                       vmr_prop_t flags)
{
        ...
        for (; cursor < len;
             cursor += PAGE_SIZE, cur_va += PAGE_SIZE, cur_pa += PAGE_SIZE) {
                ...
                PGTBL_LOG("Map 4KB page from va: %llx -> pa: %llx",
                          cur_va,
                          cur_pa);
        }
                            ...
                            vmr_prop_t flags)
{
        ...
        while (len >= PAGE_SIZE_1G) {
                ...
                set_pte_flags(entry, flags, USER_PTE);
                PGTBL_LOG("Map 1GB page from va: %llx -> pa: %llx",
                          cur_va,
                          cur_pa);
                cur_va += PAGE_SIZE_1G;
                cur_pa += PAGE_SIZE_1G;
        ...
        while (len >= PAGE_SIZE_2M) {
                ...
                set_pte_flags(entry, flags, USER_PTE);
                PGTBL_LOG("Map 2MB page from va: %llx -> pa: %llx",
                          cur_va,
                          cur_pa);
                cur_va += PAGE_SIZE_2M;
                cur_pa += PAGE_SIZE_2M;
