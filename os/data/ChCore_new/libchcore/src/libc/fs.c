...
struct ipc_msg* create_ipc_msg_with_fr(struct fs_request* fr, const void* src,
                                       size_t size)
{
        if (fr == NULL || fr->req <= FS_REQ_UNDEFINED
        ...
        size_t msg_len = (fr->req == FS_REQ_WRITE) ?
                                 sizeof(struct fs_request) + size :
        ...
        if (fr->req == FS_REQ_WRITE) {
                ipc_set_msg_data(
                        msg, (void*)src, sizeof(struct fs_request), size);
        }
...
FILE* fopen(const char* filename, const char* mode)
{
        ...
        if (ret != fr->open.new_fd) {
                if (ret < 0 && strcmp(mode, "w") == 0) {
                        ...
                        /* Create file */
                        struct fs_request* create_fr =
                                calloc(1, sizeof(struct fs_request));
                        create_fr->req = FS_REQ_CREAT;
                        create_fr->creat.mode = *(unsigned int*)mode;
                        memcpy(create_fr->creat.pathname,
                               filename,
                               strlen(filename) + 1);
                        struct ipc_msg* create_msg =
                                create_ipc_msg_with_fr(create_fr, NULL, 0);
