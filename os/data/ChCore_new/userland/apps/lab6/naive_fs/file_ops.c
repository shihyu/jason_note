...
int naive_fs_access(const char* name)
{
        ...
        while (1) {
                while (dentry_buf[end_pos] != '/' && end_pos < BLOCK_SIZE) {
                        end_pos++;
                }
                ...
                if (strcmp(name, name_buf) == 0) {
                        ...
                        //     inode_num = inode_num * 10 + (dentry_buf[i] -
                        //     '0');
                        // }
                        // return inode_num;
                }
...
int naive_fs_creat(const char* name)
{
        ...
        while (cur_pos < BLOCK_SIZE) {
                while (dentry_buf[cur_pos] != '/' && cur_pos < BLOCK_SIZE) {
                        cur_pos++;
                }
...
int naive_fs_unlink(const char* name)
{
        ...
        while (1) {
                ...
                // If not match, skip following inode num and copy them to
                // new_dentry_buf
                else {
                        while (dentry_buf[end_pos] != '/') {
                                end_pos++;
                        ...
                        memcpy(new_dentry_buf + new_dentry_buf_pos,
                               dentry_buf + start_pos,
                               end_pos - start_pos + 1);
