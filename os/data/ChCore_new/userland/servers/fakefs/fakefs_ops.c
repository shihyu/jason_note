...
void print_all_file()
{
        ...
        char* file = NULL;
        for_each_in_list (
                private_iter, struct fakefs_file_node, node, &fakefs_files) {
...
int fakefs_creat(struct ipc_msg* ipc_msg, struct fs_request* fr)
{
        ...
        struct fakefs_file_node* n = (struct fakefs_file_node*)malloc(
                sizeof(struct fakefs_file_node));
...
int fakefs_open(char* path, int flags, int mode, unsigned long* vnode_id,
...
int fakefs_write(void* operator, unsigned long offset, size_t size,
                 const char* buf)
{
        struct fakefs_file_node* file_node = (struct fakefs_file_node*)operator;
        BUG_ON(!file_node);
...
int fakefs_rmdir(const char* path, int flags)
{
        ...
        struct fakefs_file_node** del_nodes =
                malloc(sizeof(*del_nodes) * list_size);
        int nr = 0;

        for_each_in_list (
                private_iter, struct fakefs_file_node, node, &fakefs_files) {
                if (memcmp(private_iter->path, path, path_len) == 0
                    && (private_iter->path[path_len] == '/'
                        || private_iter->path[path_len] == '\0')) {
