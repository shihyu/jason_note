...
use anyhow::Result;
use buttplug::server::device::configuration::SerialSpecifier;
pub use buttplug::{
  core::message::{
...
pub fn run_engine(sink: StreamSink<String>, args: EngineOptionsExternal) -> Result<()> {
  // 使用 println! 確保輸出到終端
  println!("[INTIFACE_LOG] Rust Bridge - run_engine() 開始啟動引擎，藍芽LE設定: {}", args.use_bluetooth_le);
  ...
  info!("[INTIFACE_LOG] Rust Bridge - Frontend 日誌設定完成");
  let frontend_waiter = frontend.notify_on_creation();
  info!("[INTIFACE_LOG] Rust Bridge - 建立 IntifaceEngine 核心引擎");
...
pub fn send(msg_json: String) {
  println!("[INTIFACE_LOG] Rust Bridge - send() 收到來自 Dart 的訊息: {}", msg_json);
  info!("[INTIFACE_LOG] Rust Bridge - send() 收到來自 Dart 的訊息: {}", msg_json);
  let msg: IntifaceMessage = serde_json::from_str(&msg_json).unwrap();
  if ENGINE_BROADCASTER.receiver_count() > 0 {
    println!("[INTIFACE_LOG] Rust Bridge - 轉發訊息到引擎廣播器");
    info!("[INTIFACE_LOG] Rust Bridge - 轉發訊息到引擎廣播器");
    ENGINE_BROADCASTER
      .send(msg)
...
pub fn update_user_config(
  ...
  let dcm = DEVICE_CONFIG_MANAGER
    .try_read()
    .expect("We should have a reader at this point");
  let _ = dcm.add_user_device_definition(&identifier.into(), &config.into());
}
