...
// 儲存
#[post("/update_user")]
async fn update_user(form: web::Form<User>) -> impl Responder {
...
async fn delete_user(user_id_string: web::Path<String>) -> impl Responder {
    let mut conn = establish_connection();
...
// 以 id 查詢記錄
#[get("/user/{id}")]
async fn get_user(user_id_string: web::Path<String>) -> impl Responder {
    let mut conn = establish_connection();
...
async fn get_user_list() -> impl Responder {
    ...
    let mut user_vec: Vec<User> = users.load(&mut conn).expect("get user list error");
    // println!("{:?}", user_vec);
