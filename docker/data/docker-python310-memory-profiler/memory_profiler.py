...
def _get_memory(pid, backend, timestamps=False, include_children=False, filename=None):
    ...
    def ps_util_tool():
        ...
        try:
            ...
            if include_children:
                mem += sum(
                    [mem for (pid, mem) in _get_child_memory(process, meminfo_attr)]
                )
            if timestamps:
                return mem, time.time()
            else:
...
class TimeStamper:
    ...
    def timestamp(self, name="<block>"):
        ...
        return _TimeStamperCM(
            timestamps, filename, self.backend, timestamper=self, func=func
        )
...
def show_results(prof, stream=None, precision=1, cb_func=None):
    ...
    if True:
        ...
        for (filename, lines) in prof.code_map.items():
            ...
            tmp_list.append({"inc": "", "data": u"Filename: " + filename + "\n\n"})
            tmp_list.append({"inc": "", "data": header + u"\n"})
            tmp_list.append({"inc": "", "data": u"=" * len(header) + "\n"})
            ...
            for (lineno, mem) in lines:
                if mem:
                    ...
                    if float(inc.strip().split(" ")[0]) > 0.0:
                        inc_count += 1
                else:
                    total_mem = u""
                    inc = u""
                ...
                tmp_list.append({"inc": inc, "data": tmp})
                # stream.write(tmp)

        if inc_count > 1:
            result = []
            tmp_list_inc_count = 0
            for tmp in tmp_list:
                if tmp_list_inc_count == inc_count:
                    break
                if "" != tmp["inc"] and float(tmp["inc"].strip().split(" ")[0]) > 0.0:
                    tmp_list_inc_count += 1
                stream.write(tmp["data"])
...
class MemoryProfilerMagics(Magics):
    ...
    def memit(self, line="", cell=None):
        ...
        while counter < repeat:
            counter += 1
            tmp = memory_usage(
                (_func_exec, (stmt, self.shell.user_ns)),
                timeout=timeout,
                interval=interval,
                max_usage=True,
                max_iterations=1,
        ...
        result = MemitResult(
            mem_usage, baseline, repeat, timeout, interval, include_children
        )
        ...
        if not quiet:
            if mem_usage:
                ...
                print(
                    "ERROR: could not read memory usage, try with a "
...
def profile(func=None, stream=None, precision=1, cb_func=None, backend="psutil"):
    """
    ...
    if func is not None:
        ...
        show_results_bound = partial(
            show_results, stream=stream, precision=precision, cb_func=cb_func
        )
        if iscoroutinefunction(func):
            ...
            def wrapper(*args, **kwargs):
                # print('11111111111111111111', args, kwargs)
                prof = get_prof()
            ...
            @wraps(wrapped=func)
            def wrapper(*args, **kwargs):
                # print('22222222222222222222', args, "\n3333333333333333333" , kwargs, type(args), type(kwargs))
                prof = get_prof()
        ...
        def inner_wrapper(f):
            return profile(
                f, stream=stream, precision=precision, cb_func=cb_func, backend=backend
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        action="store",
        choices=["tracemalloc", "psutil", "psutil_pss", "psutil_uss", "posix"],
        default="psutil",
