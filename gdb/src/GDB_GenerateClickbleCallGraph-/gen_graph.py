# Author:
#     Modified by : Jaysh Patel  jnpatel81@gmail.com
# Description:
...
import sys
import pydotplus
sequence_count=1; # Sequence Counter
...
<![CDATA[
        ...
        function edgeClick(element) {
            //var path = element.getElementsByTagName('path');
            var inner_polygon = element.getElementsByTagName('polygon');
            var inner_text = element.getElementsByTagName('text');
            //path[0].setAttribute("style", "fill: yellow");
            inner_polygon[0].setAttribute("style", "fill: lightcoral");
            inner_text[0].setAttribute("style", "fill: lightcoral");
        }
        ...
        window.addEventListener('load',function(){
            ...
            edges = document.getElementsByClassName("edge");
            for(var i = edges.length - 1; i >= 0; i--) {
                edges[i].setAttribute("onclick", 'edgeClick(this)');
            }
            constructGraph();
        })
]]>
    ...
    for bt in backtraces:
        ...
        for fn in bt[1:]:
            if (fn, fn_old) not in edge_tuples:
                ...
                first_last_call[(fn,fn_old)]=('\"{}\"'.format(sequence_count), '\"{}\"'.format('-'))
                sequence_count = sequence_count + 1
            else:
                #print('dup {} -> {}'.format(fn,fn_old))
                (f,l) = first_last_call[(fn,fn_old)];
                l = '\"{}\"'.format(sequence_count)
                first_last_call[(fn,fn_old)]=(f,l)
            fn_old=deepcopy(fn)
    ...
    for edge_tuple in edge_tuples:
        if edge_tuple not in l_unique_edges:
            new_tuple = ('\"{}\"'.format(edge_tuple[0]),
                         '\"{}\"'.format(edge_tuple[1]))
            (f,l) = first_last_call[edge_tuple];
            #print('\"f:{} l:{}\"'.format(f,l))
            g.add_edge(pydotplus.Edge(new_tuple,
                                      id='\"{}|{}\"'.format(
                                            new_tuple[0].strip('"'),
                                            new_tuple[1].strip('"')),
                                      label='\"({}     {})\"'.format(f.strip('"'),l.strip('"'))))
...
def process(file_in):
    ...
    g = pydotplus.Dot(graph_type='digraph',
                      graph_name='Created by: Jayesh Patel',
                      rankdir='LR',
                      simplify='True')
    #                  bgcolor='grey35')
