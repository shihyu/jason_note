...
def test_runner_charges_sell_cost_when_position_exits():
    index = pd.date_range("2024-01-05", periods=4, freq="W-FRI")
    prices = pd.DataFrame({"AAA": [100.0, 100.0, 100.0, 100.0]}, index=index)
    positions = pd.DataFrame({"AAA": [1.0, 0.0, 0.0, 0.0]}, index=index)

    runner = BacktestRunner(cost_model=TaiwanStockCostModel(discount=0.3))
    result = runner.run(positions=positions, prices=prices)

    expected_buy_cost_ratio = 0.001425 * 0.3
    expected_sell_cost_ratio = 0.001425 * 0.3 + 0.003

    assert math.isclose(result["strategy_returns"].loc[index[1]], -expected_buy_cost_ratio, rel_tol=1e-9)
    assert math.isclose(result["strategy_returns"].loc[index[2]], -expected_sell_cost_ratio, rel_tol=1e-9)
